...
class PACC(Quantifier):
    ...
    def estimate(self, X):
        ...
        for i, _class in enumerate(self.classes):
            ...
            prevalence = np.mean(scores_class)
            
            diff_tpr_fpr = (float(self.tprfpr['tpr'][0]) - float(self.tprfpr['fpr'][0]))
            prevalence = (prevalence - float(self.tprfpr['fpr'][0])) / diff_tpr_fpr #adjusted class proportion
