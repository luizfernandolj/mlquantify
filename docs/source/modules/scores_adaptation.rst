...
**Distribution Matching (DM)** quantifiers (see :ref:`distribution_matching`), which estimate test prevalences by finding the
...
distribution.  
The chosen synthetic model is then passed to an aggregative quantifier (e.g., :class:`GACC`,
...
.. code-block:: python
   ...
   q = QuaDapt(
      quantifier=GACC(RandomForestClassifier()),
